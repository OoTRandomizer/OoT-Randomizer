...
class SaveContext():
    ...
    def give_bottleAddressAndMask(self, item, count):
        for bottle_id in range(4):
            ...
            address = self.addresses['item_slot'][item_slot].address
            bit_offset = self.addresses['item_slot'][item_slot].bit_offset
            mask = self.addresses['item_slot'][item_slot].address

            return [address, bit_offset, mask, "bottle_types"]
    ...
    def give_item_addressAndMask(self, item):
        count = 1 #TBD ok?
        if item in SaveContext.bottle_types:
            return self.give_bottleAddressAndMask(item, count)
        elif item in ["Piece of Heart", "Piece of Heart (Treasure Chest Game)"]:
            pass
        elif item == "Heart Container":
            pass
            ...
            for address, value in SaveContext.save_writes_table[item].items():
                ...
                return [address_value.address, address_value.bit_offset, address_value.mask, category]
        else:
            return -1
