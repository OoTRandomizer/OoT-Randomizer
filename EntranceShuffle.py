...
def shuffle_random_entrances(worlds: list[World]) -> None:
    ...
    for world in worlds:
        ...
        if worlds[0].shuffle_dungeon_entrances:
            ...
            # not really a closed forest anymore, so specifically place Deku Tree in its vanilla location.
            if worlds[0].settings.open_forest == 'closed':
        ...
        for pool_type, entrance_pool in one_way_entrance_pools.items():
            if world.settings.open_forest == 'closed' and pool_type not in ('OverworldOneWay', 'OwlDrop'):
                ...
                # Additionally, we ensure one-way entrances can't skip Mido to enter the Deku Tree, since closed forest implies closed Deku.
        ...
        for pool_type, entrance_pool in entrance_pools.items():
            if world.settings.open_forest == 'closed' and (
