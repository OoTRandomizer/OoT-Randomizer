...
# `indefinite` should be true if the text is used in a context that can refer to other players' items (e.g. misc. location hints, but not misc. item hints)
def item_hint_text(item: Item, clearer_hint: bool, indefinite: bool) -> str:
    if item.unshuffled_dungeon_item and item.type != 'DungeonReward':
        name = item.type
    else:
        name = item.name
    cryptic_options, clear_text_definite, clear_text_indefinite = itemTable[name]
    if clearer_hint:
        if indefinite and item.shuffled_across_worlds and clear_text_indefinite is not None:
            return clear_text_indefinite
        if clear_text_definite is not None:
            return clear_text_definite
        return cryptic_options[0]
    else:
        return random.choice(cryptic_options)
...
itemTable: dict[str, tuple[list[str], Optional[str], Optional[str]]] = {
    'Kokiri Emerald':                                           (["a tree's farewell", "the Spiritual Stone of the Forest"], "the Kokiri Emerald", "a Kokiri Emerald"),
    'Goron Ruby':                                               (["the Gorons' hidden treasure", "the Spiritual Stone of Fire"], "the Goron Ruby", "a Goron Ruby"),
    'Zora Sapphire':                                            (["an engagement ring", "the Spiritual Stone of Water"], "the Zora Sapphire", "a Zora Sapphire"),
    'Light Medallion':                                          (["a sagely power frozen in time", "an old man's sagely power", "a yellow disc"], "the Light Medallion", "a Light Medallion"),
    'Forest Medallion':                                         (["a sagely power given by a childhood friend", "a Kokiri's sagely power", "a green disc"], "the Forest Medallion", "a Forest Medallion"),
    'Fire Medallion':                                           (["a sagely power forged in lava", "a Goron's sagely power", "a red disc"], "the Fire Medallion", "a Fire Medallion"),
    'Water Medallion':                                          (["a sagely power given by your fiancée", "a Zora's sagely power", "a blue disc"], "the Water Medallion", "a Water Medallion"),
    'Shadow Medallion':                                         (["a sagely power forged in blood", "a Sheikah's sagely power", "a purple disc"], "the Shadow Medallion", "a Shadow Medallion"),
    'Spirit Medallion':                                         (["a sagely power tainted by mind control", "a Gerudo's sagely power", "an orange disc"], "the Spirit Medallion", "a Spirit Medallion"),
    'Triforce Piece':                                           (["a triumph fork", "cheese", "a gold fragment"], "a Piece of the Triforce", None),
    'Magic Meter':                                              (["mystic training", "pixie dust", "a green rectangle"], "a Magic Meter", None),
    'Double Defense':                                           (["a white outline", "damage decrease", "strengthened love"], "Double Defense", None),
    'Slingshot':                                                (["a seed shooter", "a rubberband", "a child's catapult"], "a Slingshot", None),
    'Boomerang':                                                (["a banana", "a stun stick"], "the Boomerang", "a Boomerang"),
    'Bow':                                                      (["an archery enabler", "a danger dart launcher"], "a Bow", None),
    'Bomb Bag':                                                 (["an explosive container", "a blast bag"], "a Bomb Bag", None),
    'Progressive Hookshot':                                     (["Dampé's keepsake", "the Grapple Beam", "the BOING! chain"], "a Hookshot", None),
    'Progressive Strength Upgrade':                             (["power gloves", "metal mittens", "the heavy lifty"], "a Strength Upgrade", None),
    'Progressive Scale':                                        (["a deeper dive", "a piece of Zora"], "a Zora Scale", None),
    'Megaton Hammer':                                           (["the dragon smasher", "the metal mallet", "the heavy hitter"], "the Megaton Hammer", "a Megaton Hammer"),
    'Iron Boots':                                               (["sink shoes", "clank cleats"], "the Iron Boots", "a pair of Iron Boots"),
    'Hover Boots':                                              (["butter boots", "sacred slippers", "spacewalkers"], "the Hover Boots", "a pair of Hover Boots"),
    'Kokiri Sword':                                             (["a butter knife", "a starter slasher", "a switchblade"], "the Kokiri Sword", "a Kokiri Sword"),
    'Giants Knife':                                             (["a fragile blade", "a breakable cleaver"], "the Giant's Knife", "a Giant's Knife"),
    'Biggoron Sword':                                           (["the biggest blade", "a colossal cleaver"], "the Biggoron Sword", "a Biggoron Sword"),
    'Master Sword':                                             (["evil's bane"], "the Master Sword", "a Master Sword"),
    'Deku Shield':                                              (["a wooden ward", "a burnable barrier"], "a Deku Shield", None),
    'Hylian Shield':                                            (["a steel safeguard", "Like Like's metal meal"], "a Hylian Shield", None),
    'Mirror Shield':                                            (["the reflective rampart", "Medusa's weakness", "a silvered surface"], "the Mirror Shield", "a Mirror Shield"),
    'Farores Wind':                                             (["teleportation", "a relocation rune", "a green ball", "a green gust"], "Farore's Wind", None),
    'Nayrus Love':                                              (["a safe space", "an impregnable aura", "a blue barrier", "a blue crystal"], "Nayru's Love", None),
    'Dins Fire':                                                (["an inferno", "a heat wave", "a red ball"], "Din's Fire", None),
    'Fire Arrows':                                              (["the furnace firearm", "the burning bolts", "a magma missile"], "the Fire Arrows", "Fire Arrows"),
    'Ice Arrows':                                               (["the refrigerator rocket", "the frostbite bolts", "an iceberg maker"], "the Ice Arrows", "Ice Arrows"),
    'Blue Fire Arrows':                                         (["the icy hot rocket", "the blue bolts", "an iceberg destroyer"], "the Blue Fire Arrows", "Blue Fire Arrows"),
    'Light Arrows':                                             (["the shining shot", "the luminous launcher", "Ganondorf's bane", "the lighting bolts"], "the Light Arrows", "Light Arrows"),
    'Lens of Truth':                                            (["a lie detector", "a ghost tracker", "true sight", "a detective's tool"], "the Lens of Truth", "a Lens of Truth"),
    'Ocarina':                                                  (["a flute", "a music maker"], "an Ocarina", None),
    'Goron Tunic':                                              (["ruby robes", "fireproof fabric", "cooking clothes"], "a Goron Tunic", None),
    'Zora Tunic':                                               (["a sapphire suit", "scuba gear", "a swimsuit"], "a Zora Tunic", None),
    'Epona':                                                    (["a horse", "a four legged friend"], "Epona", "an Epona"),
    'Zeldas Lullaby':                                           (["a song of royal slumber", "a triforce tune"], "Zelda's Lullaby", "a Zelda's Lullaby"),
    'Eponas Song':                                              (["an equestrian etude", "Malon's melody", "a ranch song"], "Epona's Song", "an Epona's Song"),
    'Sarias Song':                                              (["a song of dancing Gorons", "Saria's phone number"], "Saria's Song", "a Saria's Song"),
    'Suns Song':                                                (["Sunny Day", "the ReDead's bane", "the Gibdo's bane"], "the Sun's Song", "a Sun's Song"),
    'Song of Time':                                             (["a song 7 years long", "the tune of ages"], "the Song of Time", "a Song of Time"),
    'Song of Storms':                                           (["Rain Dance", "a thunderstorm tune", "windmill acceleration"], "the Song of Storms", "a Song of Storms"),
    'Minuet of Forest':                                         (["the song of tall trees", "an arboreal anthem", "a green spark trail"], "the Minuet of Forest", "a Minuet of Forest"),
    'Bolero of Fire':                                           (["a song of lethal lava", "a red spark trail", "a volcanic verse"], "the Bolero of Fire", "a Bolero of Fire"),
    'Serenade of Water':                                        (["a song of a damp ditch", "a blue spark trail", "the lake's lyric"], "the Serenade of Water", "a Serenade of Water"),
    'Requiem of Spirit':                                        (["a song of sandy statues", "an orange spark trail", "the desert ditty"], "the Requiem of Spirit", "a Requiem of Spirit"),
    'Nocturne of Shadow':                                       (["a song of spooky spirits", "a graveyard boogie", "a haunted hymn", "a purple spark trail"], "the Nocturne of Shadow", "a Nocturne of Shadow"),
    'Prelude of Light':                                         (["a luminous prologue melody", "a yellow spark trail", "the temple traveler"], "the Prelude of Light", "a Prelude of Light"),
    'Bottle':                                                   (["a glass container", "an empty jar", "encased air"], "a Bottle", None),
    'Rutos Letter':                                             (["a call for help", "the note that Mweeps", "an SOS call", "a fishy stationery"], "Ruto's Letter", "a Ruto's Letter"),
    'Bottle with Milk':                                         (["cow juice", "a white liquid", "a baby's breakfast"], "a Milk Bottle", None),
    'Bottle with Red Potion':                                   (["a vitality vial", "a red liquid"], "a Red Potion Bottle", None),
    'Bottle with Green Potion':                                 (["a magic mixture", "a green liquid"], "a Green Potion Bottle", None),
    'Bottle with Blue Potion':                                  (["an ailment antidote", "a blue liquid"], "a Blue Potion Bottle", None),
    'Bottle with Fairy':                                        (["an imprisoned fairy", "an extra life"], "a Fairy Bottle", None),
    'Bottle with Fish':                                         (["an aquarium", "a deity's snack"], "a Fish Bottle", None),
    'Bottle with Blue Fire':                                    (["a conflagration canteen", "an icemelt jar"], "a Blue Fire Bottle", None),
    'Bottle with Bugs':                                         (["an insectarium", "Skulltula finders"], "a Bug Bottle", None),
    'Bottle with Poe':                                          (["a spooky ghost", "a face in the jar"], "a Poe Bottle", None),
    'Bottle with Big Poe':                                      (["the spookiest ghost", "a sidequest spirit"], "a Big Poe Bottle", None),
    'Stone of Agony':                                           (["the shake stone", "the Rumble Pak (TM)"], "the Stone of Agony", "a Stone of Agony"),
    'Gerudo Membership Card':                                   (["a girl club membership", "a desert tribe's pass"], "the Gerudo Card", "a Gerudo Card"),
    'Progressive Wallet':                                       (["a mo' money holder", "a gem purse", "a portable bank"], "a Wallet", None),
    'Deku Stick Capacity':                                      (["a lumber rack", "more flammable twigs"], "Deku Stick Capacity", None),
    'Deku Nut Capacity':                                        (["more nuts", "flashbang storage"], "Deku Nut Capacity", None),
    'Heart Container':                                          (["a lot of love", "a Valentine's gift", "a boss's organ"], "a Heart Container", None),
    'Piece of Heart':                                           (["a little love", "a broken heart"], "a Piece of Heart", None),
    'Piece of Heart (Treasure Chest Game)':                     (["a victory valentine"], "a Piece of Heart", None),
    'Recovery Heart':                                           (["a free heal", "a hearty meal", "a Band-Aid"], "a Recovery Heart", None),
    'Rupee (Treasure Chest Game) (1)':                          (["the green gem of grief"], "a Green Rupee", None),
    'Rupees (Treasure Chest Game) (5)':                         (["the blue gem of blunder"], "a Blue Rupee", None),
    'Rupees (Treasure Chest Game) (20)':                        (["the red gem of regret"], "a Red Rupee", None),
    'Rupees (Treasure Chest Game) (50)':                        (["the purple gem of punishment"], "a Purple Rupee", None),
    'Deku Stick (1)':                                           (["a breakable branch"], "a Deku Stick", None),
    'Rupee (1)':                                                (["a unique coin", "a penny", "a green gem"], "a Green Rupee", None),
    'Rupees (5)':                                               (["a common coin", "a blue gem"], "a Blue Rupee", None),
    'Rupees (20)':                                              (["couch cash", "a red gem"], "a Red Rupee", None),
    'Rupees (50)':                                              (["big bucks", "a purple gem", "wealth"], "a Purple Rupee", None),
    'Rupees (200)':                                             (["a juicy jackpot", "a yellow gem", "a giant gem", "great wealth"], "a Huge Rupee", None),
    'Weird Egg':                                                (["a chicken dilemma"], "the Weird Egg", "a Weird Egg"),
    'Chicken':                                                  (["a chicken dilemma"], "the Chicken", "a Chicken"),
    'Zeldas Letter':                                            (["an autograph", "royal stationery", "royal snail mail"], "Zelda's Letter", "a Zelda's Letter"),
    'Keaton Mask':                                              (["the famous façade"], "the Keaton Mask", "a Keaton Mask"),
    'Skull Mask':                                               (["the fleshless façade"], "the Skull Mask", "a Skull Mask"),
    'Spooky Mask':                                              (["the frightening façade"], "the Spooky Mask", "a Spooky Mask"),
    'Bunny Hood':                                               (["the fast façade"], "the Bunny Hood", "a Bunny Hood"),
    'Goron Mask':                                               (["the fraternal façade"], "the Goron Mask", "a Goron Mask"),
    'Zora Mask':                                                (["the fishy façade"], "the Zora Mask", "a Zora Mask"),
    'Gerudo Mask':                                              (["the feminine façade"], "the Gerudo Mask", "a Gerudo Mask"),
    'Mask of Truth':                                            (["the factual façade"], "the Mask of Truth", "a Mask of Truth"),
    'Pocket Egg':                                               (["a Cucco container", "a Cucco, eventually", "a fowl youth"], "the Pocket Egg", "a Pocket Egg"),
    'Pocket Cucco':                                             (["a little clucker"], "the Pocket Cucco", "a Pocket Cucco"),
    'Cojiro':                                                   (["a cerulean capon"], "Cojiro", "a Cojiro"),
    'Odd Mushroom':                                             (["a powder ingredient"], "an Odd Mushroom", None),
    'Odd Potion':                                               (["Granny's goodies"], "an Odd Potion", None),
    'Poachers Saw':                                             (["a tree killer"], "the Poacher's Saw", "a Poacher's Saw"),
    'Broken Sword':                                             (["a shattered slicer"], "the Broken Sword", "a Broken Sword"),
    'Prescription':                                             (["a pill pamphlet", "a doctor's note"], "the Prescription", "a Prescription"),
    'Eyeball Frog':                                             (["a perceiving polliwog"], "the Eyeball Frog", "an Eyeball Frog"),
    'Eyedrops':                                                 (["a vision vial"], "the Eyedrops", "Eyedrops"),
    'Claim Check':                                              (["a three day wait"], "the Claim Check", "a Claim Check"),
    'Map':                                                      (["a dungeon atlas", "blueprints"], "a Map", None),
    'Map (Deku Tree)':                                          (["an atlas of an ancient tree", "blueprints of an ancient tree"], "a Map of the Deku Tree", "a Map of a Deku Tree"),
    'Map (Dodongos Cavern)':                                    (["an atlas of an immense cavern", "blueprints of an immense cavern"], "a Map of Dodongo's Cavern", "a Map of a Dodongo's Cavern"),
    'Map (Jabu Jabus Belly)':                                   (["an atlas of the belly of a deity", "blueprints of the belly of a deity"], "a Map of Jabu Jabu's Belly", "a Map of a Jabu Jabu's Belly"),
    'Map (Forest Temple)':                                      (["an atlas of a deep forest", "blueprints of a deep forest"], "a Map of the Forest Temple", "a Map of a Forest Temple"),
    'Map (Fire Temple)':                                        (["an atlas of a high mountain", "blueprints of a high mountain"], "a Map of the Fire Temple", "a Map of a Fire Temple"),
    'Map (Water Temple)':                                       (["an atlas of a vast lake", "blueprints of a vast lake"], "a Map of the Water Temple", "a Map of a Water Temple"),
    'Map (Shadow Temple)':                                      (["an atlas of the house of the dead", "blueprints of the house of the dead"], "a Map of the Shadow Temple", "a Map of a Shadow Temple"),
    'Map (Spirit Temple)':                                      (["an atlas of the goddess of the sand", "blueprints of the goddess of the sand"], "a Map of the Spirit Temple", "a Map of a Spirit Temple"),
    'Map (Bottom of the Well)':                                 (["an atlas of a shadow's prison", "blueprints of a shadow's prison"], "a Map of the Bottom of the Well", "a Map of the Bottom of a Well"),
    'Map (Ice Cavern)':                                         (["an atlas of a frozen maze", "blueprints of a frozen maze"], "a Map of the Ice Cavern", "a Map of an Ice Cavern"),
    'Compass':                                                  (["a treasure tracker", "a magnetic needle"], "a Compass", None),
    'Compass (Deku Tree)':                                      (["a treasure tracker for an ancient tree", "a magnetic needle for an ancient tree"], "a Deku Tree Compass", None),
    'Compass (Dodongos Cavern)':                                (["a treasure tracker for an immense cavern", "a magnetic needle for an immense cavern"], "a Dodongo's Cavern Compass", None),
    'Compass (Jabu Jabus Belly)':                               (["a treasure tracker for the belly of a deity", "a magnetic needle for the belly of a deity"], "a Jabu Jabu's Belly Compass", None),
    'Compass (Forest Temple)':                                  (["a treasure tracker for a deep forest", "a magnetic needle for a deep forest"], "a Forest Temple Compass", None),
    'Compass (Fire Temple)':                                    (["a treasure tracker for a high mountain", "a magnetic needle for a high mountain"], "a Fire Temple Compass", None),
    'Compass (Water Temple)':                                   (["a treasure tracker for a vast lake", "a magnetic needle for a vast lake"], "a Water Temple Compass", None),
    'Compass (Shadow Temple)':                                  (["a treasure tracker for the house of the dead", "a magnetic needle for the house of the dead"], "a Shadow Temple Compass", None),
    'Compass (Spirit Temple)':                                  (["a treasure tracker for a goddess of the sand", "a magnetic needle for a goddess of the sand"], "a Spirit Temple Compass", None),
    'Compass (Bottom of the Well)':                             (["a treasure tracker for a shadow's prison", "a magnetic needle for a shadow's prison"], "a Bottom of the Well Compass", None),
    'Compass (Ice Cavern)':                                     (["a treasure tracker for a frozen maze", "a magnetic needle for a frozen maze"], "an Ice Cavern Compass", None),
    'BossKey':                                                  (["a master of unlocking", "a dungeon's master pass"], "a Boss Key", None),
    'GanonBossKey':                                             (["a master of unlocking", "a dungeon's master pass"], "a Boss Key", None),
    'SmallKey':                                                 (["a tool for unlocking", "a dungeon pass", "a lock remover", "a lockpick"], "a Small Key", None),
    'HideoutSmallKey':                                          (["a get out of jail free card"], "a Jail Key", None),
    'TCGSmallKey':                                              (["a key to becoming a winner"], "a Game Key", None),
    'SmallKeyRing':                                             (["a toolbox for unlocking", "a dungeon season pass", "a jingling ring", "a skeleton key"], "a Small Key Ring", None),
    'HideoutSmallKeyRing':                                      (["a deck of get out of jail free cards"], "a Jail Key Ring", None),
    'TCGSmallKeyRing':                                          (["the keys to becoming a winner"], "a Game Key Ring", None),
    'SilverRupee':                                              (["an entry fee", "a priced artifact"], "a Silver Rupee", None),
    'Boss Key (Forest Temple)':                                 (["a master of unlocking for a deep forest", "a master pass for a deep forest"], "the Forest Temple Boss Key", "a Forest Temple Boss Key"),
    'Boss Key (Fire Temple)':                                   (["a master of unlocking for a high mountain", "a master pass for a high mountain"], "the Fire Temple Boss Key", "a Fire Temple Boss Key"),
    'Boss Key (Water Temple)':                                  (["a master of unlocking for under a vast lake", "a master pass for under a vast lake"], "the Water Temple Boss Key", "a Water Temple Boss Key"),
    'Boss Key (Shadow Temple)':                                 (["a master of unlocking for the house of the dead", "a master pass for the house of the dead"], "the Shadow Temple Boss Key", "a Shadow Temple Boss Key"),
    'Boss Key (Spirit Temple)':                                 (["a master of unlocking for a goddess of the sand", "a master pass for a goddess of the sand"], "the Spirit Temple Boss Key", "a Spirit Temple Boss Key"),
    'Boss Key (Ganons Castle)':                                 (["a master of unlocking for a conquered citadel", "a floating dungeon's master pass"], "Ganon's Castle Boss Key", "a Ganon's Castle Boss Key"),
    'Small Key (Forest Temple)':                                (["a tool for unlocking a deep forest", "a dungeon pass for a deep forest", "a lock remover for a deep forest", "a lockpick for a deep forest"], "a Forest Temple Small Key", None),
    'Small Key (Fire Temple)':                                  (["a tool for unlocking a high mountain", "a dungeon pass for a high mountain", "a lock remover for a high mountain", "a lockpick for a high mountain"], "a Fire Temple Small Key", None),
    'Small Key (Water Temple)':                                 (["a tool for unlocking a vast lake", "a dungeon pass for under a vast lake", "a lock remover for under a vast lake", "a lockpick for under a vast lake"], "a Water Temple Small Key", None),
    'Small Key (Shadow Temple)':                                (["a tool for unlocking the house of the dead", "a dungeon pass for the house of the dead", "a lock remover for the house of the dead", "a lockpick for the house of the dead"], "a Shadow Temple Small Key", None),
    'Small Key (Spirit Temple)':                                (["a tool for unlocking a goddess of the sand", "a dungeon pass for a goddess of the sand", "a lock remover for a goddess of the sand", "a lockpick for a goddess of the sand"], "a Spirit Temple Small Key", None),
    'Small Key (Bottom of the Well)':                           (["a tool for unlocking a shadow's prison", "a dungeon pass for a shadow's prison", "a lock remover for a shadow's prison", "a lockpick for a shadow's prison"], "a Bottom of the Well Small Key", None),
    'Small Key (Gerudo Training Ground)':                       (["a tool for unlocking the test of thieves", "a dungeon pass for the test of thieves", "a lock remover for the test of thieves", "a lockpick for the test of thieves"], "a Gerudo Training Ground Small Key", None),
    'Small Key (Ganons Castle)':                                (["a tool for unlocking a conquered citadel", "a dungeon pass for a conquered citadel", "a lock remover for a conquered citadel", "a lockpick for a conquered citadel"], "a Ganon's Castle Small Key", None),
    'Small Key (Thieves Hideout)':                              (["a get out of jail free card"], "a Jail Key", None),
    'Small Key (Treasure Chest Game)':                          (["a key to becoming a winner"], "a Game Key", None),
    'Small Key Ring (Forest Temple)':                           (["a toolbox for unlocking a deep forest", "a dungeon season pass for a deep forest", "a jingling ring for a deep forest", "a skeleton key for a deep forest"], "a Forest Temple Small Key Ring", None),
    'Small Key Ring (Fire Temple)':                             (["a toolbox for unlocking a high mountain", "a dungeon season pass for a high mountain", "a jingling ring for a high mountain", "a skeleton key for a high mountain"], "a Fire Temple Small Key Ring", None),
    'Small Key Ring (Water Temple)':                            (["a toolbox for unlocking a vast lake", "a dungeon season pass for under a vast lake", "a jingling ring for under a vast lake", "a skeleton key for under a vast lake"], "a Water Temple Small Key Ring", None),
    'Small Key Ring (Shadow Temple)':                           (["a toolbox for unlocking the house of the dead", "a dungeon season pass for the house of the dead", "a jingling ring for the house of the dead", "a skeleton key for the house of the dead"], "a Shadow Temple Small Key Ring", None),
    'Small Key Ring (Spirit Temple)':                           (["a toolbox for unlocking a goddess of the sand", "a dungeon season pass for a goddess of the sand", "a jingling ring for a goddess of the sand", "a skeleton key for a goddess of the sand"], "a Spirit Temple Small Key Ring", None),
    'Small Key Ring (Bottom of the Well)':                      (["a toolbox for unlocking a shadow's prison", "a dungeon season pass for a shadow's prison", "a jingling ring for a shadow's prison", "a skeleton key for a shadow's prison"], "a Bottom of the Well Small Key Ring", None),
    'Small Key Ring (Gerudo Training Ground)':                  (["a toolbox for unlocking the test of thieves", "a dungeon season pass for the test of thieves", "a jingling ring for the test of thieves", "a skeleton key for the test of thieves"], "a Gerudo Training Ground Small Key Ring", None),
    'Small Key Ring (Ganons Castle)':                           (["a toolbox for unlocking a conquered citadel", "a dungeon season pass for a conquered citadel", "a jingling ring for a conquered citadel", "a skeleton key for a conquered citadel"], "a Ganon's Castle Small Key Ring", None),
    'Small Key Ring (Thieves Hideout)':                         (["a deck of get out of jail free cards"], "a Jail Key Ring", None),
    'Small Key Ring (Treasure Chest Game)':                     (["the keys to becoming a winner"], "a Game Key Ring", None),
    'Silver Rupee (Dodongos Cavern Staircase)':                 (["an entry fee for an immense cavern", "a priced artifact from an immense cavern"], "a Silver Rupee for Dodongo's Cavern", "a Silver Rupee for a Dodongo's Cavern"),
    'Silver Rupee (Ice Cavern Spinning Scythe)':                (["an entry fee for a frozen maze", "a priced artifact from a frozen maze"], "a Silver Rupee for the Ice Cavern", "a Silver Rupee for an Ice Cavern"),
    'Silver Rupee (Ice Cavern Push Block)':                     (["an entry fee for a frozen maze", "a priced artifact from a frozen maze"], "a Silver Rupee for the Ice Cavern", "a Silver Rupee for an Ice Cavern"),
    'Silver Rupee (Bottom of the Well Basement)':               (["an entry fee for a shadow's prison", "a priced artifact from a shadow's prison"], "a Silver Rupee for the Bottom of the Well", "a Silver Rupee for the Bottom of a Well"),
    'Silver Rupee (Shadow Temple Scythe Shortcut)':             (["an entry fee for the house of the dead", "a priced artifact from the house of the dead"], "a Silver Rupee for the Shadow Temple", "a Silver Rupee for a Shadow Temple"),
    'Silver Rupee (Shadow Temple Invisible Blades)':            (["an entry fee for the house of the dead", "a priced artifact from the house of the dead"], "a Silver Rupee for the Shadow Temple", "a Silver Rupee for a Shadow Temple"),
    'Silver Rupee (Shadow Temple Huge Pit)':                    (["an entry fee for the house of the dead", "a priced artifact from the house of the dead"], "a Silver Rupee for the Shadow Temple", "a Silver Rupee for a Shadow Temple"),
    'Silver Rupee (Shadow Temple Invisible Spikes)':            (["an entry fee for the house of the dead", "a priced artifact from the house of the dead"], "a Silver Rupee for the Shadow Temple", "a Silver Rupee for a Shadow Temple"),
    'Silver Rupee (Gerudo Training Ground Slopes)':             (["an entry fee for the test of thieves", "a priced artifact from the test of thieves"], "a Silver Rupee for the Gerudo Training Ground", "a Silver Rupee for a Gerudo Training Ground"),
    'Silver Rupee (Gerudo Training Ground Lava)':               (["an entry fee for the test of thieves", "a priced artifact from the test of thieves"], "a Silver Rupee for the Gerudo Training Ground", "a Silver Rupee for a Gerudo Training Ground"),
    'Silver Rupee (Gerudo Training Ground Water)':              (["an entry fee for the test of thieves", "a priced artifact from the test of thieves"], "a Silver Rupee for the Gerudo Training Ground", "a Silver Rupee for a Gerudo Training Ground"),
    'Silver Rupee (Spirit Temple Child Early Torches)':         (["an entry fee for a goddess of the sand", "a priced artifact from a goddess of the sand"], "a Silver Rupee for the Spirit Temple", "a Silver Rupee for a Spirit Temple"),
    'Silver Rupee (Spirit Temple Adult Boulders)':              (["an entry fee for a goddess of the sand", "a priced artifact from a goddess of the sand"], "a Silver Rupee for the Spirit Temple", "a Silver Rupee for a Spirit Temple"),
    'Silver Rupee (Spirit Temple Lobby and Lower Adult)':       (["an entry fee for a goddess of the sand", "a priced artifact from a goddess of the sand"], "a Silver Rupee for the Spirit Temple", "a Silver Rupee for a Spirit Temple"),
    'Silver Rupee (Spirit Temple Sun Block)':                   (["an entry fee for a goddess of the sand", "a priced artifact from a goddess of the sand"], "a Silver Rupee for the Spirit Temple", "a Silver Rupee for a Spirit Temple"),
    'Silver Rupee (Spirit Temple Adult Climb)':                 (["an entry fee for a goddess of the sand", "a priced artifact from a goddess of the sand"], "a Silver Rupee for the Spirit Temple", "a Silver Rupee for a Spirit Temple"),
    'Silver Rupee (Ganons Castle Spirit Trial)':                (["an entry fee for a conquered citadel", "a priced artifact from a conquered citadel"], "a Silver Rupee for Ganon's Castle", "a Silver Rupee for a Ganon's Castle"),
    'Silver Rupee (Ganons Castle Light Trial)':                 (["an entry fee for a conquered citadel", "a priced artifact from a conquered citadel"], "a Silver Rupee for Ganon's Castle", "a Silver Rupee for a Ganon's Castle"),
    'Silver Rupee (Ganons Castle Fire Trial)':                  (["an entry fee for a conquered citadel", "a priced artifact from a conquered citadel"], "a Silver Rupee for Ganon's Castle", "a Silver Rupee for a Ganon's Castle"),
    'Silver Rupee (Ganons Castle Shadow Trial)':                (["an entry fee for a conquered citadel", "a priced artifact from a conquered citadel"], "a Silver Rupee for Ganon's Castle", "a Silver Rupee for a Ganon's Castle"),
    'Silver Rupee (Ganons Castle Water Trial)':                 (["an entry fee for a conquered citadel", "a priced artifact from a conquered citadel"], "a Silver Rupee for Ganon's Castle", "a Silver Rupee for a Ganon's Castle"),
    'Silver Rupee (Ganons Castle Forest Trial)':                (["an entry fee for a conquered citadel", "a priced artifact from a conquered citadel"], "a Silver Rupee for Ganon's Castle", "a Silver Rupee for a Ganon's Castle"),
    'Silver Rupee Pouch (Dodongos Cavern Staircase)':           (["a silver lining for an immense cavern", "a stash of silver shekels for an immense cavern"], "a Pouch of Silver Rupees for Dodongo's Cavern", "a Pouch of Silver Rupees for a Dodongo's Cavern"),
    'Silver Rupee Pouch (Ice Cavern Spinning Scythe)':          (["a silver lining for a frozen maze", "a stash of silver shekels for a frozen maze"], "a Pouch of Silver Rupees for the Ice Cavern", "a Pouch of Silver Rupees for an Ice Cavern"),
    'Silver Rupee Pouch (Ice Cavern Push Block)':               (["a silver lining for a frozen maze", "a stash of silver shekels for a frozen maze"], "a Pouch of Silver Rupees for the Ice Cavern", "a Pouch of Silver Rupees for an Ice Cavern"),
    'Silver Rupee Pouch (Bottom of the Well Basement)':         (["a silver lining for a shadow's prison", "a stash of silver shekels for a shadow's prison"], "a Pouch of Silver Rupees for the Bottom of the Well", "a Pouch of Silver Rupees for the Bottom of a Well"),
    'Silver Rupee Pouch (Shadow Temple Scythe Shortcut)':       (["a silver lining for the house of the dead", "a stash of silver shekels for the house of the dead"], "a Pouch of Silver Rupees for the Shadow Temple", "a Pouch of Silver Rupees for a Shadow Temple"),
    'Silver Rupee Pouch (Shadow Temple Invisible Blades)':      (["a silver lining for the house of the dead", "a stash of silver shekels for the house of the dead"], "a Pouch of Silver Rupees for the Shadow Temple", "a Pouch of Silver Rupees for a Shadow Temple"),
    'Silver Rupee Pouch (Shadow Temple Huge Pit)':              (["a silver lining for the house of the dead", "a stash of silver shekels for the house of the dead"], "a Pouch of Silver Rupees for the Shadow Temple", "a Pouch of Silver Rupees for a Shadow Temple"),
    'Silver Rupee Pouch (Shadow Temple Invisible Spikes)':      (["a silver lining for the house of the dead", "a stash of silver shekels for the house of the dead"], "a Pouch of Silver Rupees for the Shadow Temple", "a Pouch of Silver Rupees for a Shadow Temple"),
    'Silver Rupee Pouch (Gerudo Training Ground Slopes)':       (["a silver lining for the test of thieves", "a stash of silver shekels for the test of thieves"], "a Pouch of Silver Rupees for the Gerudo Training Ground", "a Pouch of Silver Rupees for a Gerudo Training Ground"),
    'Silver Rupee Pouch (Gerudo Training Ground Lava)':         (["a silver lining for the test of thieves", "a stash of silver shekels for the test of thieves"], "a Pouch of Silver Rupees for the Gerudo Training Ground", "a Pouch of Silver Rupees for a Gerudo Training Ground"),
    'Silver Rupee Pouch (Gerudo Training Ground Water)':        (["a silver lining for the test of thieves", "a stash of silver shekels for the test of thieves"], "a Pouch of Silver Rupees for the Gerudo Training Ground", "a Pouch of Silver Rupees for a Gerudo Training Ground"),
    'Silver Rupee Pouch (Spirit Temple Child Early Torches)':   (["a silver lining for a goddess of the sand", "a stash of silver shekels for a goddess of the sand"], "a Pouch of Silver Rupees for the Spirit Temple", "a Pouch of Silver Rupees for a Spirit Temple"),
    'Silver Rupee Pouch (Spirit Temple Adult Boulders)':        (["a silver lining for a goddess of the sand", "a stash of silver shekels for a goddess of the sand"], "a Pouch of Silver Rupees for the Spirit Temple", "a Pouch of Silver Rupees for a Spirit Temple"),
    'Silver Rupee Pouch (Spirit Temple Lobby and Lower Adult)': (["a silver lining for a goddess of the sand", "a stash of silver shekels for a goddess of the sand"], "a Pouch of Silver Rupees for the Spirit Temple", "a Pouch of Silver Rupees for a Spirit Temple"),
    'Silver Rupee Pouch (Spirit Temple Sun Block)':             (["a silver lining for a goddess of the sand", "a stash of silver shekels for a goddess of the sand"], "a Pouch of Silver Rupees for the Spirit Temple", "a Pouch of Silver Rupees for a Spirit Temple"),
    'Silver Rupee Pouch (Spirit Temple Adult Climb)':           (["a silver lining for a goddess of the sand", "a stash of silver shekels for a goddess of the sand"], "a Pouch of Silver Rupees for the Spirit Temple", "a Pouch of Silver Rupees for a Spirit Temple"),
    'Silver Rupee Pouch (Ganons Castle Spirit Trial)':          (["a silver lining for a conquered citadel", "a stash of silver shekels for a conquered citadel"], "a Pouch of Silver Rupees for Ganon's Castle", "a Pouch of Silver Rupees for a Ganon's Castle"),
    'Silver Rupee Pouch (Ganons Castle Light Trial)':           (["a silver lining for a conquered citadel", "a stash of silver shekels for a conquered citadel"], "a Pouch of Silver Rupees for Ganon's Castle", "a Pouch of Silver Rupees for a Ganon's Castle"),
    'Silver Rupee Pouch (Ganons Castle Fire Trial)':            (["a silver lining for a conquered citadel", "a stash of silver shekels for a conquered citadel"], "a Pouch of Silver Rupees for Ganon's Castle", "a Pouch of Silver Rupees for a Ganon's Castle"),
    'Silver Rupee Pouch (Ganons Castle Shadow Trial)':          (["a silver lining for a conquered citadel", "a stash of silver shekels for a conquered citadel"], "a Pouch of Silver Rupees for Ganon's Castle", "a Pouch of Silver Rupees for a Ganon's Castle"),
    'Silver Rupee Pouch (Ganons Castle Water Trial)':           (["a silver lining for a conquered citadel", "a stash of silver shekels for a conquered citadel"], "a Pouch of Silver Rupees for Ganon's Castle", "a Pouch of Silver Rupees for a Ganon's Castle"),
    'Silver Rupee Pouch (Ganons Castle Forest Trial)':          (["a silver lining for a conquered citadel", "a stash of silver shekels for a conquered citadel"], "a Pouch of Silver Rupees for Ganon's Castle", "a Pouch of Silver Rupees for a Ganon's Castle"),
    'KeyError':                                                 (["something mysterious", "an unknown treasure"], "An Error (Please Report This)", None),
    'Arrows (5)':                                               (["a few danger darts", "a few sharp shafts"], "Arrows (5 pieces)", None),
    'Arrows (10)':                                              (["some danger darts", "some sharp shafts"], "Arrows (10 pieces)", None),
    'Arrows (30)':                                              (["plenty of danger darts", "plenty of sharp shafts"], "Arrows (30 pieces)", None),
    'Bombs (5)':                                                (["a few explosives", "a few blast balls"], "Bombs (5 pieces)", None),
    'Bombs (10)':                                               (["some explosives", "some blast balls"], "Bombs (10 pieces)", None),
    'Bombs (20)':                                               (["lots-o-explosives", "plenty of blast balls"], "Bombs (20 pieces)", None),
    'Ice Trap':                                                 (["a gift from Ganon", "a chilling discovery", "frosty fun"], "an Ice Trap", None),
    'Magic Bean':                                               (["a wizardly legume"], "a Magic Bean", None),
    'Buy Magic Bean':                                           (["a wizardly legume"], "a Magic Bean", None),
    'Magic Bean Pack':                                          (["wizardly legumes"], "Magic Beans", None),
    'Bombchus':                                                 (["mice bombs", "proximity mice", "wall crawlers", "trail blazers"], "Bombchus", None),
    'Bombchus (5)':                                             (["a few mice bombs", "a few proximity mice", "a few wall crawlers", "a few trail blazers"], "Bombchus (5 pieces)", None),
    'Bombchus (10)':                                            (["some mice bombs", "some proximity mice", "some wall crawlers", "some trail blazers"], "Bombchus (10 pieces)", None),
    'Bombchus (20)':                                            (["plenty of mice bombs", "plenty of proximity mice", "plenty of wall crawlers", "plenty of trail blazers"], "Bombchus (20 pieces)", None),
    'Deku Nuts (5)':                                            (["some nuts", "some flashbangs", "some scrub spit"], "Deku Nuts (5 pieces)", None),
    'Deku Nuts (10)':                                           (["lots-o-nuts", "plenty of flashbangs", "plenty of scrub spit"], "Deku Nuts (10 pieces)", None),
    'Deku Seeds (30)':                                          (["catapult ammo", "lots-o-seeds"], "Deku Seeds (30 pieces)", None),
    'Gold Skulltula Token':                                     (["proof of destruction", "an arachnid chip", "spider remains", "one percent of a curse"], "a Gold Skulltula Token", None),
    'Ocarina A Button':                                         (["a blue note"], "the Ocarina A Button", "an Ocarina A Button"),
    'Ocarina C up Button':                                      (["a high note"], "the Ocarina C up Button", "an Ocarina C up Button"),
    'Ocarina C down Button':                                    (["a low note"], "the Ocarina C down Button", "an Ocarina C down Button"),
    'Ocarina C left Button':                                    (["a somewhat high note"], "the Ocarina C left Button", "an Ocarina C left Button"),
    'Ocarina C right Button':                                   (["a middle note"], "the Ocarina C right Button", "an Ocarina C right Button"),
    'Fairy Drop':                                               (["an annoying companion", "Navi's cousin"], "a Stray Fairy", None),
    'Nothing':                                                  (["emptiness", "loneliness"], "Nothing", None),
}
